...
export class AsteroidManager extends Phaser.Events.EventEmitter {
  ...
  public destroyAsteroid(asteroid: Asteroid, causeSplit = true): AsteroidData[] {
    if (!this.activeAsteroids.has(asteroid)) {
      return [];
    }
  ...
  public getPoolStats(): Record<string, { poolSize: number; activeCount: number }> {
    const stats: Record<string, { poolSize: number; activeCount: number }> = {};

    for (const [size, pool] of this.pools.entries()) {
      const activeCount = Array.from(this.activeAsteroids).filter(
        (a) => a.getSize() === size
      ).length;
      stats[size] = {
