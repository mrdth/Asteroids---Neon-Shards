...
export class ShardManager extends Phaser.Events.EventEmitter {
  ...
  private readonly onShardCollectedHandler: (event: {
    shard: Shard;
    value: number;
    position: { x: number; y: number };
  }) => void;
  private readonly onShardExpiredHandler: (event: {
    shard: Shard;
    position: { x: number; y: number };
  }) => void;
  ...
  public update(dt: number, playerPosition: { x: number; y: number }): void {
    ...
    for (const shard of this.activeShards) {
      if (shard.getIsActive()) {
        shard.update(dt, playerPosition, this.config.magnetRadius, this.config.magnetForce);
      }
    }
  }
  ...
  public checkCollisions(playerShip: Phaser.Physics.Arcade.Sprite): void {
    ...
    for (const shard of this.activeShards) {
      if (shard.getIsActive()) {
        const distance = Phaser.Math.Distance.Between(playerShip.x, playerShip.y, shard.x, shard.y);
